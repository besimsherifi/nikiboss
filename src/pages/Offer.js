...
function Offer() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    window.open(`mailto:info@besi-storen.ch?subject=Offerte&body=${formData.message}    von ${formData.name} ${formData.lastname} - ${formData.number}`)
    setFormData({
      name: '',
      lastname: '',
      email: '',
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid gap-6 mb-6 md:grid-cols-2">
            ...
            <div data-aos='fade-right'>
              ...
              <input id="lastname" name="number" value={formData.number}
                onChange={handleChange} type="name" placeholder='+41' required className="block w-full rounded-md border-0 py-1.5 px-1 text-gray-900 bg-gray-100 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400  sm:text-sm sm:leading-6"></input>
            </div>
            <div data-aos='fade-left'>
          ...
          <div data-aos='fade-up'>
            ...
            <textarea id="message" name='message' onChange={handleChange} rows="4" className="block p-2.5 w-full text-sm text-gray-900 mb-6 bg-gray-100 rounded-lg border border-gray-300 " placeholder="Sonnenstore 170(cm)h x 60(cm)b"></textarea>
          </div>



          <button type="submit" onSubmit={handleSubmit} data-aos='fade-right' className="text-white bg-custom-orange hover:bg-blue-400 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center ">{t('FORM.SENDEN')}</button>
        </form>
      </div>
    </>
  );
